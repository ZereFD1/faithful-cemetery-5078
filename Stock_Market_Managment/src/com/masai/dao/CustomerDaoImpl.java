 ...
 *         query check if result set is empty
 */
...
public class CustomerDaoImpl implements CustomerDao {
	...
	@Override
	public void buyStock(int cust_id, int stock_id, int buyquantity)
...
//					check if customer has enough amount in wallet and if have then update wallet amount
					...
					String update_wallet_amount = "UPDATE customer set wallet = '" + updated_wal_amt + "'where cid = '"
							+ cust_id + "'";
					...
					String update_stock_quantity = "UPDATE stocks set quantity = '" + updated_stk_quan
							+ "'where sid = '" + stock_id + "'";
...
//					check if already in table then update quantity else add new field in table
					String custStockMap_SELECT_QUERY = "SELECT * FROM customer_stock_mapping where cid = '" + cust_id
							+ "' && sid = '" + stock_id + "'";
...
//						String updatequanityOfstockid = "UPDATE customer_stock_mapping set quantity ="+ newquanityOfstockid +  "where cid = "+cust_id+" &&  sid ="+ stock_id;
						...
						PreparedStatement psupdatequanityOfstockid = connection
								.prepareStatement(updatequanityOfstockid);
...
//					Adding buy stock info in transaction table
	...
	@Override
	public void sellStock(int cust_id, int stock_id, int sellquantity)
			throws SomeThingWrongException, NoRecordFoundException {
		...
		try {
			...
			connection = DbUtils.connectToDatabase();
			// prepare the query
			String Stock_SELECT_QUERY = "SELECT * FROM customer_stock_mapping where cid = '" + cust_id + "' && sid = '"
					+ stock_id + "' ";
...
//						also add back stock in stocks quantity in stocks table;
						...
						while (rsaddStockInStockstable.next()) {
							...
							PreparedStatement ps_updateStkQuantityInStocksTable = connection
									.prepareStatement(updateStkQuantityInStocksTable);
...
//							add sell stock in transaction table also;
							...
							if (ps_transaction.executeUpdate() > 0) {
								...
								System.out.println(
										"Thank you For Selling stock,Money is added in your Wallet Successfully.");
								return;
							} else {
								System.out.println("Unable to purchase because of some error");
								return;
	...
	public void viewTransactionHistory(int customer_id) throws SomeThingWrongException, NoRecordFoundException {
		...
		try {
			...
			connection = DbUtils.connectToDatabase();
			// prepare the query
			String SELECT_QUERY = "select stock_name,type,transaction.quantity from transaction inner join stocks on transaction.sid = stocks.sid where cid = '"
					+ customer_id + "' ";
			;
			...
			while (resultSet.next()) {
				System.out.println("Stock Name = " + resultSet.getString(1) + " Type = " + resultSet.getString(2)
						+ " Quanity = " + resultSet.getInt(3));
			}
